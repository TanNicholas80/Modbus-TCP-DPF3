        ...
        try {
            ...
            for (const machine of machines) {
                try {
                    ...
                    const now = Date.now();
                    const isHeartbeatDue = (now - (state.lastHeartbeatWrite || 0) > 30000);

                    // --- 2. TULIS STATUS ALARM (Address 100) ---
                    const addrAlarm = machine.addressAlarm || 100;
                    if (state.targetAlarm !== state.lastWrittenAlarm || isHeartbeatDue) {
                        await sleep(50);
                        ...
                        console.log(`[PLC WRITE SUCCESS${isHeartbeatDue ? ' - HEARTBEAT' : ''}] Machine ${machine.apiMachineId} Alarm (Addr ${addrAlarm}) -> ${state.targetAlarm}`);
                    }

                    // --- 3. TULIS BARCODE LENGKAP / MAINTENANCE MANUAL END (Address 103) ---
                    const addr103 = machine.addressCompleteOrMaint || 103;
                    if (state.target103 !== state.lastWritten103 || isHeartbeatDue) {
                        await sleep(50);
                        if (!client.isOpen) throw new Error("Koneksi terputus sebelum menulis data");
                        
                        await client.writeRegister(addr103, state.target103);
                        state.lastWritten103 = state.target103;
                        console.log(`[PLC WRITE SUCCESS${isHeartbeatDue ? ' - HEARTBEAT' : ''}] Machine ${machine.apiMachineId} Barcode Complete/Maint End (Addr ${addr103}) -> ${state.target103}`);
                    }

                    // --- 4. TULIS STATUS PINJAM MESIN (Address 105) ---
                    const addr105 = machine.addressPinjamMesin || 105;
                    if (state.target105 !== state.lastWritten105 || isHeartbeatDue) {
                        await sleep(50);
                        if (!client.isOpen) throw new Error("Koneksi terputus sebelum menulis data");
                        
                        await client.writeRegister(addr105, state.target105);
                        state.lastWritten105 = state.target105;
                        console.log(`[PLC WRITE SUCCESS${isHeartbeatDue ? ' - HEARTBEAT' : ''}] Machine ${machine.apiMachineId} Pinjam Mesin (Addr ${addr105}) -> ${state.target105}`);
                    }

                    if (isHeartbeatDue) {
                        state.lastHeartbeatWrite = now;
                    }
